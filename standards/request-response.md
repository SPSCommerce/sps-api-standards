...
This is based off a currently [unaccepted RFC header proposal](https://datatracker.ietf.org/doc/html/draft-idempotency-header-01).
...
- The header value **MUST NOT** be longer than 255 characters.
- An API **MUST** return a `400` when the header value is an invalid format.
- If an action is idempotent, an API **MUST** return a `2xx` on conflict.
- If an action is not idempotent, an API **MUST** return a `409` on conflict.
